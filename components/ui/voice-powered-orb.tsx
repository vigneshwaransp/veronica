  ...
  useEffect(() => {
    ...
    try {
      ...
      const update = (t: number) => {
        ...
        program.uniforms.hue.value = (hue + (t * 0.015)) % 360;
        ...
        if (enableVoiceControl && isMicrophoneInitialized) {
          ...
          // Default idle animations when voice is disabled
          currentRot += dt * (baseRotationSpeed + maxRotationSpeed * 0.4);
          program.uniforms.hover.value = 0.5; // active pulse
          program.uniforms.hoverIntensity.value = maxHoverIntensity * 0.65; // scale based on prop
          if (onVoiceDetected) {
